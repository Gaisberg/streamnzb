...
rule reads: what the name says, what the indexer tagged the release as, and
the audio tracks ffprobe read when the file has been opened, unioned. A dub
posted under an untouched English name and tagged Arabic only in the feed
satisfies a required Arabic and is caught by an excluded one, rather than
