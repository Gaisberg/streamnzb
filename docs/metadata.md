...
- **Language** — Titles, overviews, episode names, logos and catalog rows display in the language picked on the Metadata page. TMDB content localizes fully; TVDB series overlay TheTVDB's translations where they exist. Anime keeps its Kitsu titles but picks up a translated description through the anime-lists mapping when the mapped record has one. Anything without a translation falls back to English, and clients cache title pages for a few hours, so a language change reaches already-visited titles after their cache expires. Independent of the per-search-query title language, which controls what gets sent to indexers.
