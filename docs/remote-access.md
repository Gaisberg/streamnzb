...
- **Do not publish StreamNZB's port** (`ports:` in compose, `-p` on the command line) when this is on. Docker's port forwarding rewrites every outside connection to the bridge gateway address, which sits inside the network you just trusted, so a published port turns "the proxy" into "anyone on the internet". The same holds for a proxy on `127.0.0.1` if the port is also bound on loopback. Let the proxy reach StreamNZB over the Docker network only. StreamNZB logs the trusted list at startup so it can be checked against the listen address.
- **The proxy must strip the identity header from every request it does not authenticate.** Bypass rules are the trap: Stremio addon paths need them, and a bypassed request still arrives from the proxy's address. Authelia behind Traefik's `forwardAuth` is safe — Traefik deletes each `authResponseHeaders` name before copying Authelia's answer, on bypassed requests too. Caddy's `forward_auth copy_headers` and nginx's `auth_request_set` only copy headers that are present and leave the client's own copy alone on a bypass, so with those add an unconditional delete at the edge (`request_header -Remote-User` in Caddy, `proxy_set_header Remote-User "";` before the auth block in nginx). Whatever the proxy, keep `/api/` out of every bypass rule.
...
A request that presents its own credential — `Authorization: Bearer` or `?token=` — is judged on that credential alone and never lifted to the admin by the proxy. `trustForwardHeader` makes Traefik pass the client's `X-Forwarded-*` headers to Authelia; that is only safe when the entry point's `forwardedHeaders.trustedIPs` names the proxy in front of Traefik, or when Traefik itself is the edge and the setting is left off. Authelia writes `Remote-User` on every request it lets through and strips whatever the client sent, which is exactly the property this relies on. Authentik uses `X-authentik-username`; oauth2-proxy uses `X-Forwarded-User` when `--set-xauthrequest` is on. Put that header name in `trusted_proxy_auth_header`.
