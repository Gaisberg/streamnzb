...
- **Streams** — A named profile that binds providers, indexers, search requests and filter profiles into one configuration, with its own playback history and its own credentials. Stremio and Jellyfin are two equal ways of consuming it — see [How clients consume a stream](#how-clients-consume-a-stream). A stream can be renamed at any time: both the Stremio manifest URL (`<token>/manifest.json`) and a signed-in Jellyfin session key on the token, not the name, so installed addons and signed-in clients keep working; the next Jellyfin sign-in uses the new name. Playback history moves with the stream.

## How clients consume a stream

| Client | How it connects |
|---|---|
| Stremio | Install `<token>/manifest.json` as an addon. |
| Jellyfin | Sign in with the stream name as username and its token or password as password. |
...
- **Resource Selections** — Which providers, indexers, search queries, and filter profiles belong to the stream.
...
- **Per-provider connection caps** — Optionally limit how many of a provider's connections this stream may hold during playback, so one stream cannot monopolise the account. It is a ceiling, not a reservation: it stops a stream taking everything, but does not hold connections back for anyone else. The [provider speed test](speed-test.md) tells you the floor — the connection count each resolution needs.
...
- **Results Mode** — Whether each release is listed separately or collapsed into one combined entry. Stremio-only: Jellyfin always offers every ranked release as its own media source, capped by `JELLYFIN_MAX_PLAYBACK_SOURCES` — see [Jellyfin endpoint](jellyfin.md). Which releases are offered, and the order they come in, lives in the filter profile — see [Filters & ranking](filters.md).
...
- **Metadata Profile** — Which catalogs, display language and rating limit the stream serves, on both clients. None means the classic stream-only addon on Stremio; on Jellyfin it leaves nothing to browse, since libraries are catalogs — see [Metadata & Catalogs](metadata.md).
- **Format Profile** — Stremio-only: how the stream's results render as Stremio result rows. None means the built-in format — see [Custom result formats](result-formatting.md).
