...
**A map an article disproves is thrown away.** Every downloaded article is checked against the length the map claims for it. A disagreement means some length in the map was inferred rather than measured, and serving through it would shift every byte after it — so the read is refused (`refusing to serve shifted bytes`), the measured size that taught it is forgotten, and the map is rebuilt from real articles on the next read (`Discarded a segment map disproved by a decoded article`). Before that, one wrong size could keep a file unreadable for as long as it stayed open, which reached the player as a video that never decoded rather than a failover.
...
**A filled hole is rewritten so the player skips it.** Zeros are not neutral inside a Matroska file: they sit where the demuxer expects an element header, and 0x00 is not a valid one. Lenient players scan forward and recover; strict ones (ExoPlayer- and Infuse-class) stop on the spot, which is why a release could play for ten minutes and then die on a seek. So before the bytes go out, a hole inside an `.mkv` or `.webm` stream is rewritten into `Void` elements — the structure a demuxer skips by size — keeping the byte count identical, so every offset after the hole stays where the container says it is. In practice the damage becomes a skipped fraction of a second instead of a stopped player. It happens automatically, with nothing to configure; the log records each repair as `Matroska hole rewritten as void elements`. Nothing else is touched: MP4 streams, compressed or password-protected archives, and any stream whose holes cannot be located are served exactly as before, and so is a hole the repair cannot make sense of — logged as `Matroska hole fill fell back to the original bytes`. The repair reduces what a hole costs a player; it does not make a damaged release whole, and the limits above still decide when a release is refused.
...
Expand the request on **History**. The search panel at the top walks the funnel
from what each indexer returned to what the client got, the play attempts
follow it, and under **Turned away before playback** is every release that
never got that far — one line each. Expand a release to see what stopped it:

- **validation** — the release name did not match the requested title, year,
  season or episode. The badges name the check and what it expected against
  what the name said, plus the search request it was answering.
  `expected "lioness", got "Special Ops Lioness"` is the answer to most of
  these: scene names keep prefixes the metadata title drops, and the validation
  query comes from metadata.
- **known bad** — every copy of the release has failed playback before, so it
  was retired. This is the "it worked yesterday" case.
- **a profile reason** — the filter profile's traits, limits, languages and
  named rules. A `rule:` badge means one of your own rules did it, which is a
  different place to look than a blocked quality.

The first eight are listed; **Show N more turned away** reveals the rest. A
search answered with hundreds of wrong results records only the first 50 per
request by name, and says how many more there were once the full list is open.
The counts in the panel above are always complete.
