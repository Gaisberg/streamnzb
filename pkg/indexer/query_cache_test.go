...
func (m *mockIndexer) Search(req SearchRequest) (*SearchResponse, error) {
	m.searchCalls++
...
func TestQueryCacheDifferentQueryParamsMiss(t *testing.T) {
	...
	_, _ = cached.Search(req1)
	if mock.searchCalls != 1 {
		t.Fatalf("expected 1 search call, got %d", mock.searchCalls)
	}

	_, _ = cached.Search(req2)
...
func (s *slowMockIndexer) Search(req SearchRequest) (*SearchResponse, error) {
	if s.onSearch != nil {
		s.onSearch()
	}
	time.Sleep(s.delay)
...
func (s *slowMockIndexer) Ping() error {
	return nil
}
...
func TestQueryCacheConcurrentInFlightDeduplication(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		res1, _ = cached.Search(req1)
	}()
	go func() {
		defer wg.Done()
		res2, _ = cached.Search(req2)
	}()
