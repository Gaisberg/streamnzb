...
// resultLanguages is every language a result is known to carry: the audio
// tracks a probe read, the indexer's own tag and what jhin parsed out of the
// name, resolved by the same function the rules and the Stremio stream use,
// so the three read one answer.
