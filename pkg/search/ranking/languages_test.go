...
// The file's own audio tracks are the third account, and count the same way:
// a German track a probe read out of an untouched English release satisfies a
// German requirement.
func TestLanguagesRequiredSeesProbedTracks(t *testing.T) {
	p := languageProfile(t, rank.Languages{Required: []string{"de"}}, nil)
	cand := triage.Candidate{
		Release: &release.Release{Title: plainTitle},
		Verdict: triage.Verdict{Probed: &release.MediaCaps{TracksProbed: true, AudioLanguages: []string{"de"}}},
	}
	kept, rejected := p.ApplyWithRejected(ranking.Request{Kind: ranking.KindMovie}, []triage.Candidate{cand}, rank.RankOptions{})
	if len(kept) != 1 {
		t.Fatalf("release probed German rejected for %v, want kept", rejected)
	}
	// A probe that did not read the tracks says nothing about them.
	cand.Verdict.Probed = &release.MediaCaps{AudioLanguages: []string{"de"}}
	if kept, _ := p.ApplyWithRejected(ranking.Request{Kind: ranking.KindMovie}, []triage.Candidate{cand}, rank.RankOptions{}); len(kept) != 0 {
		t.Fatalf("untracked probe satisfied the requirement, want rejected")
	}
}
