...
func TestCommitGoodAttemptIfQualifiedBelowThreshold(t *testing.T) {
	...
	if committed := server.commitGoodAttemptIfQualified(nil, sess, sess.ID, time.Now().Add(-5*time.Second)); committed {
...
func TestCommitGoodAttemptIfQualifiedCommitsAtThreshold(t *testing.T) {
	...
	if committed := server.commitGoodAttemptIfQualified(nil, bytesOnly, bytesOnly.ID, time.Now()); committed {
	...
	if committed := server.commitGoodAttemptIfQualified(nil, durationOnly, durationOnly.ID, time.Now().Add(-21*time.Second)); committed {
	...
	if committed := server.commitGoodAttemptIfQualified(nil, sustained, sustained.ID, time.Now().Add(-21*time.Second)); !committed {
...
func TestCommitGoodAttemptIfQualifiedUsesAvailThresholds(t *testing.T) {
	...
	if committed := server.commitGoodAttemptIfQualified(nil, sess, sess.ID, time.Now()); committed {
		t.Fatal("bytes alone must not commit success before the duration threshold")
	}

	// Both custom thresholds met: commits.
	if committed := server.commitGoodAttemptIfQualified(nil, sess, sess.ID, time.Now().Add(-time.Minute)); !committed {
