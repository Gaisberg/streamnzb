...
// and fails every required-subtitle filter (issue #283). The tier is
// "probe" once the file's tracks were read, "indexer" until then.
func streamMediaInfo(languages, subtitles []string, probed *release.MediaCaps) *StreamMediaInfo {
	...
	return &StreamMediaInfo{Quality: quality, Languages: languages, Subtitles: subtitles}
}
