...
func TestBuildStreamsSurfacesSubtitlesInParsedMediaInfo(t *testing.T) {
	...
	// Once the file's tracks were read the block is measured, not reported.
	probed := languageCandidate("Movie.2024.1080p.WEB-DL.x264-GRP")
	probed.Verdict.Probed = &release.MediaCaps{TracksProbed: true, SubtitleLanguages: []string{"ar"}}
	list = &playlistResult{IsAIOStreams: true, Candidates: []triage.Candidate{probed}}
	if got := buildStreamsFromPlaylist(list, key, "Standalone", DefaultServiceName, "http://host", true, nil); got[0].MediaInfo == nil || got[0].MediaInfo.Quality != "probe" {
		t.Errorf("probed release carries %+v, want mediaInfoQuality probe", got[0].MediaInfo)
	}
