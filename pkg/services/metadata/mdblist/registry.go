...
// Registry hands out one client per stream, because an MDBList account belongs
// to a person rather than to the server. Clients are kept rather than rebuilt
// per request so a renewed access token is not lost to an instance still
// holding the one it replaced.
...
// the instance when the id did not change.
...
// stream can start the device flow.
...
// LinkedStreams lists the streams with an account on file, sorted.
